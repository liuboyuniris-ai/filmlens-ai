...
const ShotHeader = ({ shot, analysis, t }: any) => (
  <div className="flex items-center gap-3 mb-6">
    ...
    <span className="font-bold text-[15px] text-gray-900">
      {t('scene')} {shot?.shot_id || '-'}{analysis?.shot_scale ? ` · ${analysis.shot_scale}` : ''} {shot ? ` · ${shot.duration.toFixed(0)}${t('seconds')}` : ''}
    </span>
  </div>
)
      ...
      {analysis.theoretical_connections.map((tItem: any, i: number) => (
        ...
        >
          <div className="font-bold text-[#5B4FCF] text-[15px] leading-tight mb-1">
            {tItem.theory_name_en} · <span className="text-[13px] font-medium opacity-80">{tItem.theory_name_cn}</span>
          </div>
          <div className="text-[12px] text-gray-500 mb-3 italic">
            {tItem.theorist_en} ({tItem.year}) {tItem.theorist_cn && `· ${tItem.theorist_cn}`}
          </div>
  ...
  if (!displayedShotId) {
    return (
      <div className="w-[380px] border-l border-gray-100 bg-white flex flex-col items-center justify-center text-center p-10">
        ...
        <p className="text-[15px] font-bold text-gray-900 mb-2">{t('ready')}</p>
        <p className="text-[13px] text-gray-400 leading-relaxed whitespace-pre-line">{t('readySub')}</p>
      </div>
    )
  }
  ...
  return (
    <div className="w-[380px] shrink-0 border-l border-gray-100 bg-white flex flex-col h-full shadow-2xl relative z-10">
      ...
      <div className="flex border-b border-gray-100 px-4 shrink-0 bg-white/80 backdrop-blur-md sticky top-0 overflow-x-auto hide-scrollbar">
        <TabButton type="ANALYSIS" label={t('tabAnalysis')} />
        <TabButton type="RESEARCH" label={t('tabResearch')} />
        <TabButton type="OVERVIEW" label={t('tabOverview')} />
        <TabButton type="INQUIRY" label={t('tabInquiry')} />
      </div>
      ...
      <div className={`flex-1 overflow-y-auto hide-scrollbar transition-all duration-200 ${isFading ? 'opacity-0 translate-y-2' : 'opacity-100 translate-y-0'}`}>
        <div className="p-6">
          {activeTab === 'ANALYSIS' && (
            analysis ? (
              <ErrorBoundary fallbackMessage={t('renderFailed')}>
          ...
          {activeTab === 'OVERVIEW' && (
            filmContext ? (
              <div className="space-y-6">
                ...
                <div className="space-y-6">
                  <div>
                    <h3 className="text-[11px] text-gray-400 font-bold uppercase tracking-widest mb-3">{t('visualSummary')}</h3>
                  ...
                  <div className="grid grid-cols-1 gap-4">
                    {[
                      { label: t('politicalLabel'), content: filmContext.political, icon: 'M3 21v-4m0 0V5a2 2 0 012-2h6.5l1 1H21l-3 6 3 6h-8.5l-1-1H5a2 2 0 00-2 2zm9-13.5V9' },
                      { label: t('economicLabel'), content: filmContext.economic, icon: 'M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z' },
                      { label: t('culturalLabel'), content: filmContext.cultural, icon: 'M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253' },
                      { label: t('genderLabel'), content: filmContext.gender_sexuality, icon: 'M12 4.354l1.1 3.383h3.558l-2.877 2.09 1.1 3.383-2.881-2.09-2.881 2.09 1.1-3.383-2.877-2.09h3.558L12 4.354z' }
                    ].map((item, idx) => (
              ...
              <div className="py-20 text-center animate-pulse">
                ...
                <h4 className="font-bold text-gray-900 mb-2">{t('generatingReport')}</h4>
                <p className="text-xs text-gray-400 px-4">{t('generatingReportSub')}</p>
              </div>
            )
          )}
            ...
            <div className="space-y-4">
              {chatMessages.length === 0 ? (
                <div className="h-full flex flex-col justify-center items-center py-20 opacity-40">
                  ...
                  <p className="text-sm font-medium text-center whitespace-pre-line">{t('inquiryPlaceholder')}</p>
                </div>
              ) : (
                <div className="space-y-6">
                  {chatMessages.map((msg) => (
                    <div key={msg.id} className={`flex flex-col ${msg.role === 'user' ? 'items-end' : 'items-start'}`}>
                      ...
                      <span className="text-[10px] text-gray-400 mt-1 px-1">
                        ...
                        {msg.shotId && ` · ${t('basedOnShot', { id: msg.shotId })}`}
                      </span>
                    </div>
                  ))}
                  <div ref={chatEndRef} />
